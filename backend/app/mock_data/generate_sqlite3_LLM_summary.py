...
def get_accumulated_descriptions():
    try:
        ...
        results = db.query(
            ...
            func.group_concat(distinct(database.UserRemindee.relationship)).label("unique_relationships"),
            func.group_concat(distinct(database.UserRemindee.summary)).label("accumulated_summary")
        ).group_by(
            database.UserRemindee.user_id,
            database.UserRemindee.person_name
        ).all()
